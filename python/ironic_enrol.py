...
def find_idrac_ports(conn):
    return [{
            ...
            "pxe_bootstrap_ip": [tag for tag in raw_port["tags"] if "10.45" in tag][0],
        }
...
# Open connection to the OpenStack cloud
conn = openstack.connection.from_config(cloud="arcus", debug=True)

# Populate IDRAC port
idracs = find_idrac_ports(conn)
print("IDRAC ports found: ", len(idracs))

# Populate list of existing baremetal servers
baremetal_nodes = find_baremetal_servers(conn)
print("Total Baremetal nodes found: ",len(baremetal_nodes))

already_created = [node["name"] for node in baremetal_nodes]

# Enroll nodes not already enrolled in Ironic, with predefined set of drivers and metadata
new_nodes = []
for idrac in idracs:
    if idrac["name"] not in already_created:
        # Prepopulate extra field for PXE network IP address
        pxe_ip = idrac["ip"].replace("45","43")
        new_node = conn.baremetal.create_node(
            name=idrac["name"], driver="idrac",
            driver_info={
              "drac_address": idrac["ip"],
              # Starting with default passwords as shipped, update all later
            ...
            extra={
               ...
               "pxe_bootstrap_ip": idrac["pxe_bootstrap_ip"]
            },
...
print("New nodes found: ", len(new_nodes))

if len(new_nodes) > 0:
    baremetal_nodes = find_baremetal_servers(conn)
print("Total baremetal nodes now: ", len(baremetal_nodes))

# If node is in enroll state, go to manage state
move_to_manage = []
...
# Perform an out-of-band inspection, if CPU data not populated
inspecting = []
for node in baremetal_nodes:
...
# Get nodes out of maintenance mode
...
# We should only power newly manageable nodes off (not any pre-existing inventory)
